...
@Service
@Slf4j
@Extension(useCase = BizScenario.HANDLE_CASE,scenario = BizScenario.FEED_TYPE_LIVEPREVIEW)
public class LivePreviewFeedHandler extends AbstractFeedHandler<FeedHandlerContext> {
    @Override
    protected void doHandle(FeedHandlerContext ctx) {
        log.info("livepreview handle");
        ctx.getRsp().put("livepreview","handle ok");
    }
}
