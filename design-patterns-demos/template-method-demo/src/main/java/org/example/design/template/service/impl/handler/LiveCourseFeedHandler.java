...
@Service
@Slf4j
@Extension(useCase = BizScenario.HANDLE_CASE,scenario = BizScenario.FEED_TYPE_LIVECOURSE)
public class LiveCourseFeedHandler extends AbstractFeedHandler<FeedHandlerContext> {
    @Override
    protected void doHandle(FeedHandlerContext ctx) {
        log.info("livecourse handle");
        ctx.getRsp().put("livecourse","handle ok");
    }
}
