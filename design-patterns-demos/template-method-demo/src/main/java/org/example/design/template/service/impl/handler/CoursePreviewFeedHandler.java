...
@Service
@Slf4j
@Extension(useCase = BizScenario.HANDLE_CASE,scenario = BizScenario.FEED_TYPE_COURSEPREVIEW)
public class CoursePreviewFeedHandler extends AbstractFeedHandler<FeedHandlerContext> {
    @Override
    protected void doHandle(FeedHandlerContext ctx) {
        log.info("coursepreview handle");
        ctx.getRsp().put("coursepreview","handle ok");
    }
}
