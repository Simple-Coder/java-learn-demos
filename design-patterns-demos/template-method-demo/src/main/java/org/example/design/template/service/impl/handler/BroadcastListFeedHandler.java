...
@Service
@Slf4j
@Extension(useCase = BizScenario.HANDLE_CASE,scenario = BizScenario.FEED_TYPE_BROACASTLIST)
public class BroadcastListFeedHandler extends AbstractFeedHandler<FeedHandlerContext> {
    @Override
    protected void doHandle(FeedHandlerContext ctx) {
        log.info("broadcastlist handle");
        ctx.getRsp().put("broadcastlist","handle ok");
    }
}
